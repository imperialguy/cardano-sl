...
data FilterOperation a =
      forall idx. IsIndexOf a idx => FilterByIndex idx
    | forall idx. IsIndexOf a idx => FilterByPredicate Ordering idx
    | FilterNoOp
...
parseFilterOperation :: forall a idx. (IsIndexOf a idx)
                     => Proxy a
                     -> Proxy idx
                     -> Text
                     -> Either Text (FilterOperation a)
...
instance (HasServer subApi ctx
         , KnownSymbol sym
         , FilterParam sym res ~ idx
         , IsIndexOf res idx
         ) => HasServer (FilterBy sym res :> subApi) ctx where
    type ServerT (FilterBy sym res :> subApi) m = FilterOperation res -> ServerT subApi m
        ...
        let querytext req = parseQueryText $ rawQueryString req
            ...
            parseParam req =
              case List.lookup paramName (querytext req) of
                ...
                Just (Just v) ->
                  case parseFilterOperation (Proxy @res) (Proxy @idx) v of
